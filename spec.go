...
type Interface interface {
	...
	NewKubeConfigForRESTConfig(ctx context.Context, config *rest.Config, clusterName string) ([]byte, error)
	// NewRESTConfigForKubeConfig returns a REST Config for the given KubeConfigValue.
	NewRESTConfigForKubeConfig(ctx context.Context, config *KubeConfigValue) (*rest.Config, error)
}
